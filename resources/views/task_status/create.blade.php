<x-app-layout>
    <x-slot name="header">
        {{ __('views.task_status.create.form_header') }}
    </x-slot>

    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8  space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg ">
            <div class="max-w-xl">
                {{ html()->modelForm($taskStatus, 'POST', route('task_statuses.store'))->class('max-w-xl')->open() }}
                {{ html()->label(__('views.task_status.create.labels.name'), 'name')->class('block mb-2 text-lg font-medium text-gray-900 dark:text-white') }}
                {{--            first:mt-0 mt-6--}}
